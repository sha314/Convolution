...
std::vector<T> list_to_vector(PyObject *list){
    ...
    for (i = 0; i < n; i++) {
        ...
        if (!PyFloat_Check(item)) continue; /* Skip non-integers */
...
std::vector<T> tuple_to_vector(PyObject *py_tuple){
    ...
    for(size_t i{}; i < len; ++i){
        ...
        if (!PyFloat_Check(item)){
            std::cout << "non-float at " << i << " : line " << __LINE__ << std::endl;
...
std::vector<std::vector<double>> get_matrix(PyObject* py_mat);
PyObject* get_python_matrix(std::vector<std::vector<double>>& mat);
